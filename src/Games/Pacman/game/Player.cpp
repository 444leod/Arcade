...
void Player::initTextures(arc::ITextureManager &manager)
{
    ...
    for (uint32_t i = 0; i < 4; i++) {
        ...
        for (uint32_t j = 0; j < 2; j++) {
            ...
            ss1 << ss.str() << "_" << j;
            #if V2
            spec.graphical = arc::TextureImage{"assets/pacman/images/pokemon_spritesheet.png", arc::Rect<uint32_t>{(i * 48 + j * 24), 96, 24, 24}};
            #else
            spec.graphical = arc::TextureImage{"assets/pacman/entity-tileset.png", arc::Rect<uint32_t>{(j * 20), (i * 20), 20, 20}};
            #endif
    ...
    #if V2
    spec.graphical = arc::TextureImage{"assets/pacman/images/pokemon_spritesheet.png", arc::Rect<uint32_t>{0, 96, 24, 24}};
    #else
